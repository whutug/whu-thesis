...
同时，\WhuThesis{} 也提供了一套修改文档信息的接口 \verb|\whusetup|。您可以在 \verb|\whusetup| 中指定或修改 \verb|info| 信息，即学号 \verb|info/student-number|、论文题目 \verb|info/title|、院系 \verb|info/school|、专业 \verb|info/major|、作者 \verb|info/author|、指导教师与职称 \verb|info/advisor| 及日期 \verb|info/date| 等七个信息。当未给出日期信息时，默认为当前编译时间点的年月的汉数字格式，即\zhdigits{\the\year}年\zhnumber{\the\month}月。
...
如果用户使用 \app{vscode} 进行写作，在目录内的 \verb|.vscode\| 文件夹内已经有配置好的 latexmk 与 \hologo{XeLaTeX}、\hologo{LuaLaTeX} 编译链，用户可直接使用。