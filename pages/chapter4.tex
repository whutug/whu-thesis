...
\subsection{代码高亮}
\pkg{minted} 包所提供的 \env{minted} 环境还可以对代码进行高亮，请参考 \href{https://www.overleaf.com/learn/latex/Code_Highlighting_with_minted}{Code Highlighting with minted}。在使用 \env{minted} 环境前，请先在\cls{whuthesis.cls} 文件中启用 \pkg{minted} 包。

注意使用 \pkg{minted} 包时，需要系统默认 \app{Python} 有 \app{Pygments} 库，可以通过 \verb|$ pip install Pygments| 来进行安装。且需要在编译时加上 \verb|--shell-escape| 参数，否则会报错。
