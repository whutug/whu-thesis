...
\section{定理的使用}
\begin{theorem}
  设向量 \(\symbfit{a} \neq \symbf{0}\)，那么向量\(\symbfit{b} \parallel \symbfit{a}\) 的充分必要条件是：存在唯一的实数 \(\lambda\)，使 \(\symbfit{b} = \lambda \symbfit{a}\)。
\end{theorem}
\begin{definition}
  这是一条定义。
\end{definition}
...
\end{lemma}
\begin{corollary}
  对数轴上任意一点 \(P\)，轴上有向线段 \(\overrightarrow{OP}\) 都可唯一地表示为点 \(P\) 的坐标与轴上单位向量 \(\symbfit{e}_u\) 的乘积：\(\overrightarrow{OP} = u \symbfit{e}_u\)。
\end{corollary}
\begin{proposition}
  这是一条性质。
\end{proposition}
